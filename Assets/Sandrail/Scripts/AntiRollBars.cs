...
public class AntiRollBars : MonoBehaviour
{
	...
	void FixedUpdate ()
	{
		...
		if (leftWheelGrounded == true) {
            transform.GetComponent<Rigidbody>().AddForceAtPosition (leftWheelCollider.transform.up * -antiRollForce, leftWheelCollider.transform.position);
		}
		
		if (rightWheelGrounded == true) {
            transform.GetComponent<Rigidbody>().AddForceAtPosition (rightWheelCollider.transform.up * antiRollForce, rightWheelCollider.transform.position);
		}
	}
}
